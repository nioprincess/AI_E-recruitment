...
const JobDialog = ({ job, isOpen, onClose }) => {
  if (!isOpen || !job) return null;
    ...
    const authAxios= useUserAxios()
  const user= useUser()
    const downloadFile = async (url, filename) => {
    try {
      const resp = await authAxios.get(url, { responseType: "blob" });
  ...
  const parseRequirements = (requirements) => {
    ...
    try {
      ...
      // If it's not valid JSON, return as plain text in 'other' field
  ...
  // Get all dynamic requirement fields
  const requirementFields = Object.keys(requirements).filter(key => 
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              <p className="text-lg text-gray-700 dark:text-gray-300">
                {job.company?.c_admin?.c_name || 'Unknown Company'}
              </p>
              <p className="text-gray-600 dark:text-gray-400">{job.j_location}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <h3 className="font-semibold text-gray-900 dark:text-white">Employment Type</h3>
              <p className="text-gray-600 dark:text-gray-400">{job.j_employement_type || 'Not specified'}</p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-900 dark:text-white">Salary Range</h3>
              <p className="text-gray-600 dark:text-gray-400">{formatSalary(job.j_salary_min, job.j_salary_max)}</p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-900 dark:text-white">Posted Date</h3>
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">{formatDate(job.j_deadline)}</p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-900 dark:text-white">Status</h3>
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                job.j_status === 'Active'.toLowerCase() 
          ...
          {/* Attachments */}
          {job.attachments && job.attachments.length > 0 && user.user_id&&(
            <div>
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 flex items-center justify-center bg-blue-100 dark:bg-blue-900 rounded">
                        <span className="text-blue-600 dark:text-blue-300 text-sm font-medium">
                          {attachment.f_type?.includes('pdf') ? 'PDF' : 
                           attachment.f_type?.includes('image') ? 'IMG' : 
                           attachment.f_type?.includes('word') ? 'DOC' : 'FILE'}
                        </span>
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {Math.round(attachment.f_size / 1024)} KB • {attachment.f_type}
                        </p>
                      </div>
                    </div>
                    <button
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">{job.company?.c_name || 'N/A'}</p>
              </div>
              <div>
                <h4 className="font-medium text-gray-900 dark:text-white">Industry</h4>
                <p className="text-gray-600 dark:text-gray-400">{job.company?.c_industry || 'N/A'}</p>
              </div>
              <div>
                <h4 className="font-medium text-gray-900 dark:text-white">Email</h4>
                <p className="text-gray-600 dark:text-gray-400">{job.company?.c_email || 'N/A'}</p>
              </div>
              <div>
                <h4 className="font-medium text-gray-900 dark:text-white">Phone</h4>
                <p className="text-gray-600 dark:text-gray-400">{job.company?.c_phone || 'N/A'}</p>
              </div>
              <div className="md:col-span-2">
                <h4 className="font-medium text-gray-900 dark:text-white">Description</h4>
                <p className="text-gray-600 dark:text-gray-400">{job.company?.c_description || 'No description available.'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end space-x-3 p-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            ...
            disabled={job.j_status !== 'Active'.toLowerCase()}
          ...
          >
            {job.j_status === 'Active'.toLowerCase() ? 'Apply Now' : 'Not Accepting Applications'}
          </button>
        </div>
      </div>
    </div>
...
const Jobs = () => {
  ...
  const companies = [...new Set(jobs.map(job => job.company?.c_name || 'Unknown Company'))];
  const locations = [...new Set(jobs.map(job => job.j_location).filter(Boolean))];
  const jobTypes = [...new Set(jobs.map(job => job.j_employement_type).filter(Boolean))];

  const filteredJobs = jobs.filter(job => {
    const matchesSearch = job.j_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      job.company?.c_name?.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesLocation = !locationFilter || job.j_location === locationFilter;
    const matchesJobType = !jobTypeFilter || job.j_employement_type === jobTypeFilter;
    const matchesCompany = !selectedCompany || job.company?.c_name === selectedCompany;
      ...
      <div id='jobs' className="bg-gray-50 dark:bg-gray-900 min-h-screen px-4 py-8 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-center items-center h-64">
            <div className="text-xl text-gray-600 dark:text-gray-400">Loading jobs...</div>
          </div>
        </div>
      </div>
    );
