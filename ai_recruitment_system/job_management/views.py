...
from file_management.tasks import  save_job_attachment, update_job_attachment, delete_job_attachment, delete_file
...
class JobViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not company:
            raise PermissionDenied("You are not allowed to create a job for any company.")
        ...
        send_notification.delay(
            message=f"New Job '{job.j_title}' posted from {company.c_name} by {user.username}",
            n_type='job_creation',
            is_read=False,
            c_id=company.id,
            user_id=user.id,
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not company_id:
            return Response({'success': False, 'message': 'You are not authorized to create a job for this company'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def perform_update(self, serializer):
        ...
        company = Company.objects.filter(c_admin=user).first()
        if not company:
            return Response({'success': False, 'message': 'You are not authorized to update this job'}, status=status.HTTP_403_FORBIDDEN)
        data = self.request.data.copy()
        if self.request.FILES:
            return Response({'success': False, 'message': 'j_attachments cannot be updated directly'}, status=status.HTTP_400_BAD_REQUEST)
       
        serializer.save(u_id=user, c_id_id=company.id)

        return Response({'success': True, 'data': serializer.data, 'message': 'Job updated successfully'}, status=status.HTTP_200_OK)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not job:
            return Response({'success': False, 'message': 'You are not authorized to delete this job'}, status=status.HTTP_403_FORBIDDEN)
    ...
    @action(detail=False, methods=['post'], permission_classes=[permissions.IsAuthenticated], url_path='add-job-attachment')
    def add_job_attachment(self, request):
        ...
        if not job:
            return Response({'success': False, 'message': 'You are not authorized to add attachments to this job'}, status=status.HTTP_403_FORBIDDEN)
        ...
        if len(files) > 5:
            return Response({'success': False, 'message': 'You can only upload a maximum of 5 attachments'}, status=status.HTTP_400_BAD_REQUEST)
        allowed_types = ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
        for file in files:
            if file.content_type not in allowed_types:
                return Response({'success': False, 'message': f"File type '{file.content_type}' is not allowed"}, status=status.HTTP_400_BAD_REQUEST)
            save_job_attachment.delay(
                file_data_bytes=file.read(),
                file_name=file.name,
                file_type=file.content_type,
        ...
        return Response({'success': True, 'message': 'Job attachments added successfully'}, status=status.HTTP_201_CREATED)

    @action(detail=False, methods=['patch'], permission_classes=[permissions.IsAuthenticated], url_path='update-job-attachment')
    def update_job_attachment(self, request):
        ...
        if not job_id or not file_id:
            return Response({'success': False, 'message': 'Job ID and File ID are required'}, status=status.HTTP_400_BAD_REQUEST)
        job = Job.objects.filter(id=job_id, u_id=user).first()
        if not job:
            return Response({'success': False, 'message': 'You are not authorized to update this job attachment'}, status=status.HTTP_403_FORBIDDEN)
        ...
        if not new_file:
            return Response({'success': False, 'message': 'New file is required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'success': True, 'message': 'Job attachment updated successfully'}, status=status.HTTP_200_OK)
    @action(detail=False, methods=['delete'], permission_classes=[permissions.IsAuthenticated], url_path='delete-job-attachment')
    def delete_job_attachment(self, request):
        ...
        job = Job.objects.filter(id=job_id, u_id=user).first()
        if not job:
            return Response({'success': False, 'message': 'You are not authorized to delete this job attachment'}, status=status.HTTP_403_FORBIDDEN)
        file = job.j_attachments.filter(id=file_id).first()
        if not file:
            return Response({'success': False, 'message': 'File not found'}, status=status.HTTP_404_NOT_FOUND)
        delete_job_attachment.delay(file_id=file.id, job_id=job.id)
        return Response({'success': True, 'message': 'Job attachment deleted successfully'}, status=status.HTTP_204_NO_CONTENT)
    ...
    @action(detail=True, methods=['get'], permission_classes=[permissions.IsAuthenticated], url_path='applications')
    ...
    @action(detail=True, methods=['post'], permission_classes=[permissions.IsAuthenticated], parser_classes=[MultiPartParser], url_path='applications')
    def submit_application(self, request, pk=None):
        ...
        if not file:
            return Response({
                'success': False,
                'message': 'Application cover letter is required.'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    @action(detail=True, methods=['delete'], permission_classes=[permissions.IsAuthenticated], url_path='applications/(?P<app_id>[^/.]+)')
